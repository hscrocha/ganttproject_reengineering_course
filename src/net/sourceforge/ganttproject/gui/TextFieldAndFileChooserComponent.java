...
//        innerBox.add(Box.createHorizontalStrut(5));
    ...
    private void examineFile(File f) {
        if (!f.exists()) {
            showFileStatus(new Status(IStatus.ERROR, "foo", myFileSelectionMode, "File does not exist", null));
            return;
        }
        if (!f.canRead()) {
            showFileStatus(new Status(IStatus.ERROR, "foo", myFileSelectionMode, "File read error", null));
            return;
        }
        showFileStatus(new Status(IStatus.OK, "foo", myFileSelectionMode, "  ", null));
    }
